...
// ConfigMap when a component includes a "database" trait.
// Acceptance criteria:
...
func TestEngine_RenderWithDatabaseTrait(t *testing.T) {
	...
	t.Run("RenderDatabaseTrait", func(t *testing.T) {
		...
		// Expect: Deployment + Service + ConfigMap = 3
		if len(objects) != 3 {
		...
		for _, obj := range objects {
			...
			if kind == "ConfigMap" && name == "api-server-db-config" {
				foundConfigMap = true
				...
				t.Error("Secret should NOT be generated by the CUE trait")
			}
		}
